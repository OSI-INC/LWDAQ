...
#
# Neurorecorder_pick allows the user to pick a new play_file, record_file, 
...
#
# Neurorecorder_command handles the various control commands generated by
...
# play.
#
proc Neurorecorder_command {target action} {
	upvar #0 Neurorecorder_info info
	global LWDAQ_Info

	set event_executing [string match "Neurorecorder_$target\*"\
		$LWDAQ_Info(current_event)]
	set event_pending 0
	foreach event $LWDAQ_Info(queue_events) {
		if {[string match "Neurorecorder_$target\*" $event]} {
			set event_pending 1
		}
	}
	...
	if {$action != $info($target\_control)} {
		if {!$event_executing} {
			...
			if {!$event_pending} {
				if {$action != "Stop"} {
					LWDAQ_post Neurorecorder_$target
				} {
			...
			if {$action != "Stop"} {
				LWDAQ_post [list Neurorecorder_command $target $action]	
			} {
				set info($target\_control) $action
			}
		}
...
proc Neurorecorder_open {} {
	...
	foreach a {Start Stop PickDir} {
		...
		button $f.$b -text $a -command "Neurorecorder_command record $a"
