<script>
# [22-MAR-23] Exercise projection routines.
LWDAQ_open BCAM
LWDAQ_acquire BCAM
...
pack $f.go $f.scanx $f.scany $f.scanz $f.fit -side left
...
proc fit {} {
	global x y z t LWDAQ_config_BCAM
	set result [lwdaq_simplex "$x $y $z" go -report 1 -max_steps 10]
	scan $result %f%f%f x y z
}
</script>
...
proc go {x y z} {
	...
	set t5 [clock milliseconds]
	# LWDAQ_print $t "[expr $t2-$t1] [expr $t3-$t2] [expr $t4-$t3] [expr $t5-$t4]"
	LWDAQ_print $t "$x $y $z $count" orange
	return "$count"
}
...
proc fit {} {
	global x y z t LWDAQ_config_BCAM
	set z10 [expr $z/10]
	set result [lwdaq_simplex "$x $y $z10" altitude -report 1 -max_steps 10]
	scan $result %f%f%f x y z
}
</script>
...
proc go {x y z} {
	...
	set t5 [clock milliseconds]
	# LWDAQ_print $t "[expr $t2-$t1] [expr $t3-$t2] [expr $t4-$t3] [expr $t5-$t4]"
	LWDAQ_print $t "[format %.3f $x] [format %.3f $y] [format %.3f $z] $count" orange
	return "$count"
}
...
proc go {x y z} {
	global t LWDAQ_config_BCAM stop_fit
...
proc fit {} {
	...
	set z10 [expr $z/10]
	set result [lwdaq_simplex "$x $y $z10" altitude -report 1 -max_steps 100]
	scan $result %f%f%f%f x y z10
	set z [format %.1 [expr $z10*10]]
	go $x $y $z
}
</script>
