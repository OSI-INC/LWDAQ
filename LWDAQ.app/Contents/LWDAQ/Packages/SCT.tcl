# Subcutaneous Transmitter Package, (c) 2007-2021 Kevan Hashemi, Open Source Instruments Inc.
#
...
# Neuroarchiver mode is "N" for full Neuroarchiver functionality. But if we 
# specify "R" we get the Recorder only, which we call a Neurorecorder, and 
# if we specify "P" we get the Player only, which we call a Neuroplayer.
#
proc Neuroarchiver_init {{mode "N"}} {
#
...
proc Neuroarchiver_open {} {
	...
	switch $info(mode) {
		"N" {
			wm title $w "Neuroarchiver $info(version)"
			wm maxsize $w [expr $x*2] [expr $y*2]
		}	
		"P" {
			wm title . "Neuroplayer $info(version)"
			wm maxsize . [expr $x*2] [expr $y*2]
		}	
		"R" {
			wm title . "Neurorecorder $info(version)"
			wm maxsize . [expr $x*2] [expr $y*2]
		}		
	}
	
	if {($info(mode) == "N") || ($info(mode) == "R")} {
		set f $w.record
		frame $f
		pack $f -side top -fill x
