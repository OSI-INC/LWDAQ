...
# Global variables we must not change. These constants we derive from the EDF
# header format definition, see http://www.edfplus.info/specs/edf.html. We
# declare only the constants that are likely to be needed by multiple routines,
# but not those that will be used only once in the header creation routine.
...
proc EDF_recording_read {file_name} {
	...
	set patient [read $f $EDF(recording_len)]
	close $f
...
# global array. The names of the signals must be integers, or else the read will
# not be compatible with our numbered channel export routines. The routine is
# guaranteed to work on EDF files generated by this EDF package. The routine
# returns a list of signal numbers and sample rates from which the calling
# routine can compose a channel selector string.f
#
proc EDF_header_read {file_name} {
	...
	set EDF(patient) [EDF_string_pop header $EDF(patient_len)]
	set EDF(recording) [EDF_string_pop header $EDF(recording_len)]
	set date [EDF_string_pop header 16]
	set size_again [EDF_string_pop header 8]
	set reserved [EDF_string_pop header 44]
	set num_records [EDF_string_pop header $EDF(num_records_len)]
	...
	foreach {p len} {transducer 80 unit 8 min 8 max 8 lo 8 hi 8 filter 80} {
		foreach id $ids {
			set EDF($p\_$id) [EDF_string_pop header $len]
		}
	}
	set signals [list]
	foreach id $ids {
